...
class Settings(BaseSettings):
    ...
    openai_api_key: SecretStr = Field(
        default=SecretStr("local-dev-key"), alias="VLLM_API_KEY"
    )
    openai_api_base: str = Field(
        default="http://localhost:8000/v1", alias="OPENAI_API_BASE"
    )
    ...
    twitter_access_secret: SecretStr | None = Field(
        default=None, alias="ACCESS_TOKEN_SECRET"
    )
